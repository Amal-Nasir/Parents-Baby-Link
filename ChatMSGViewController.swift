...
class ChatMSGViewController: JSQMessagesViewController, UIActionSheetDelegate, UINavigationControllerDelegate, UINavigationBarDelegate {
    ...
    override func viewDidLoad() {
       ...
       timer = NSTimer.scheduledTimerWithTimeInterval(0.5, target: self, selector: "loadMessages", userInfo: nil, repeats: false)
        self.senderId = PFUser.currentUser()!.objectId
        self.senderDisplayName = PFUser.currentUser()!["first_name"] as! String
        
        //query to open chat msg
        let notRead1 = PFQuery(className: "Messages")
        ...
        notRead1.findObjectsInBackgroundWithBlock { (objects: [PFObject]?, error:NSError?) -> Void in
            ...
            for objects in User {
                ...
                let read = PFQuery(className: "Messages")
                read.whereKey("from", equalTo: PFUser.currentUser()!)
                read.whereKey("to", equalTo: self.ParentID!)
